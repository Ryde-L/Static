...
export function base() {
  ...
  window.getQueryString = function getQueryString(paramName) {
    ...
    const hashSpArr = url.split(paramName + "=");
    if (hashSpArr.length >= 2) {
      const result = hashSpArr[1].split("&");
      const result2 = result[0].split("#");
      return result2[0];
    } else {
      return null;
    }
  ...
  if (getQueryString("noWeChat")) {
    ...
    function GetContext() {
      ...
      //如果不存在，说明从菜单点击过来，直接进中间件拿数据
        ...
        const BaseContextUrl = domain + "/BaseContext.aspx";
    ...
    function GetContextFromStorage() {
      ...
      if (
        new Date().getTime() > context.ExpireTime ||
        context.ExpireTime == undefined ||
    ...
    window.SetContext = function SetContext(context) {
      ...
      const AppId = veevlinkSession.getItem(sessionKey);
      const AgentId = veevlinkSession.getItem(activedAgentIdSessionKey);
      if (AgentId == null) {
        localKey = keyPrefix + AppId;
      } else {
    ...
    function ShowError(error) {
      ...
      if (context.ShowDevError) {
        if (error.responseText) {
          $("body").append(
            errorPageTemplate(
              error.responseText + " " + error.status + " " + error.statusText
            )
          );
        } else if (error.message && error.errorCode) {
          $("body").append(
            errorPageTemplate(error.message + " " + error.errorCode)
          );
        } else if (
      ...
      if (window.addEventListener) {
        const closeBtn = document.getElementById("error_close");
 ...
 */
    function GetContext() {
      ...
      const OpenId = getQueryString("OpenId");
      let isWeb = getQueryString("isWeb");
      if (isWeb == "" || isWeb == null) {
        isWeb = "false";
      }
      //全局控制是否是测试环境
      const IsTest = getQueryString("test") || veevlinkBase.IsTest;
      /**********************以下代码部署到微信环境可以删除******************************/
      if (IsTest) {
        ...
        context.RefreshTokenProxyUrl =
          "https://dev.veevlink.com/Proxy/SFDCAccessTokenProxy.aspx";
      ...
      if (code == null) {
        ...
        //如果appid不为空，拼接获取localstorage的key
          ...
          if (veevlinkStorage.hasOwnProperty(localKey)) {
            ...
            GetContextFromStorage();
            if (
              (context.OpenId == "" || context.OpenId == null) &&
              (context.WEQYUserId == "" || context.WEQYUserId == null) &&
              isWeb == "false"
            ) {
              WechatOAuth(AppId, AgentId);
            }
      ...
      //如果code不为空，说明是第一次进行身份认证，微信会带着code返回本页面
        ...
        if (!veevlinkStorage.hasOwnProperty(localKey)) {
          ...
          //var url = window.location.protocol + "//" + window.location.host;
          //拼接进行身份认证服务的url
          const BaseUrl = domain + "/Base.aspx";
          ...
          GetContextFromStorage();
          if (
            (context.OpenId == null || context.OpenId == "") &&
            (context.WEQYUserId == null || context.WEQYUserId == "") &&
            AppId != null &&
            AppId != ""
          ) {
            WechatOAuth(AppId, AgentId);
      ...
      if (
        (context.WechatAccountType == "Enterprise" ||
          context.WechatAccountType == "WeWork") &&
        context.WechatIdentityMode == "SSOIntegrated" &&
        ...
        self.location =
          domain +
          "/Wechat2SFDC/OAuth/PreOAuth.aspx?sourceid=" +
          AppId +
          "&agentid=" +
          AgentId;
      }
    ...
    function WechatOAuth(AppId, AgentId) {
      ...
      const BaseOAuthRouterUrl = domain + "/BaseOAuthRouter.aspx";
      ...
      ajax({
        ...
        success: function(res, xml) {
          ...
          if (res !== null && res !== "") {
            ...
            const oauthurl = resJson.Oauthurl;
            if (oauthurl != null) {
              //微信认证跳转卡住时进行页面刷新操作
              setTimeout(function() {
