...
//alert(this.refreshtokenProxyUrl);
   ...
   * @param [paramMap={}] parameters to send as top values for POST/PATCH etc
  ...
  forcetk.Client.prototype.apexrest =  function (path, callback, error, method, payload, paramMap, retry) {
    ...
    return $.ajax({
      ...
      error: (!retry) ? error : function (jqXHR, textStatus, errorThrown) {
        if (!window.IsTest&&(jqXHR.status === 401 || jqXHR.status === 0) && refreshtokenCount < 3) {
          refreshtokenCount++;
          /*alert(jqXHR.statusText);
          alert(jqXHR.status);
          alert(jqXHR.responseText);*/
      ...
      beforeSend: (xhr)=> {
        // debugger;
        console.log('that.sessionId',that.sessionId)
        var paramName;
        ...
        if (this.sessionId) {
          xhr.setRequestHeader(that.authzHeader, "Bearer " + this.sessionId);
        }
        //xhr.setRequestHeader('X-User-Agent', 'salesforce-toolkit-rest-javascript/' + that.apiVersion);
      }
    });
